...
# @dataclass
# class ChunkConfig:
#     chunk_size: int        # characters (swap length_function for token-based)
#     chunk_overlap: int
#     splitter: str          # "recursive" | "sentence" | "token"
...
#         default_factory=lambda: _env("INGESTION_WATCH_FOLDER", "./data")
#     )
#     chroma_persist_dir: str = field(
#         default_factory=lambda: _env("INGESTION_CHROMA_PERSIST_DIR", "./chroma_db")
#     )
#     state_db_path: str = field(
#         default_factory=lambda: _env("INGESTION_STATE_DB_PATH", "./ingestion_state.db")
#     )

#     # --- file handling ---------------------------------------------------
#     # FIX: added .tif alias so it matches the extractor's image branch
#     supported_extensions: tuple = (
#         ".pdf", ".docx", ".txt", ".md",
#         ".html", ".csv"
#     )
#     stability_wait_seconds: float = field(
#         default_factory=lambda: float(_env("INGESTION_STABILITY_WAIT", "2.0"))
#     )
#     stability_polls: int = field(
#         default_factory=lambda: int(_env("INGESTION_STABILITY_POLLS", "3"))
#     )

#     # --- embedding -------------------------------------------------------
#     embedding_model: str = field(
#         default_factory=lambda: _env("INGESTION_EMBEDDING_MODEL", "nomic-embed-text")
#     )
#     embedding_batch_size: int = field(
#         default_factory=lambda: int(_env("INGESTION_EMBEDDING_BATCH_SIZE", "8"))
#     )
...
#         default_factory=lambda: _env("INGESTION_CHROMA_COLLECTION", "documents")
#     )

#     # --- pipeline --------------------------------------------------------
#     max_retries: int = field(
#         default_factory=lambda: int(_env("INGESTION_MAX_RETRIES", "3"))
#     )
...
#     # PDFs: dense paragraphs → larger window
#     # CSVs: row-structured   → smaller window
#     # Images: OCR output is usually short → medium window
#     chunk_configs: Dict[str, ChunkConfig] = field(default_factory=lambda: {
#         "pdf":   ChunkConfig(chunk_size=1800, chunk_overlap=250, splitter="recursive"), # Dense prose
#         "docx":  ChunkConfig(chunk_size=1800, chunk_overlap=250, splitter="recursive"),
#         "txt":   ChunkConfig(chunk_size=1500, chunk_overlap=200, splitter="recursive"), # Code/plain configs
#         "md":    ChunkConfig(chunk_size=1500, chunk_overlap=200, splitter="recursive"), 
#         "html":  ChunkConfig(chunk_size=1500, chunk_overlap=200, splitter="recursive"),
#         "csv":   ChunkConfig(chunk_size=800,  chunk_overlap=80,  splitter="recursive"), # Row structures
#         "image": ChunkConfig(chunk_size=1200, chunk_overlap=150, splitter="recursive"), # Noisy OCR text
#     })

#     def log_summary(self) -> None:
#         """Emit a structured summary of the active configuration at startup."""
#         log.info(
#             "IngestionConfig loaded | watch=%s chroma=%s state_db=%s "
#             "model=%s batch=%d retries=%d log_level=%s",
#             self.watch_folder,
